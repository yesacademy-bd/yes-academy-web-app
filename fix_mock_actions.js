...
const regexCreate = /export async function createMockService\(formData: FormData\) \{[\s\S]*?revalidatePath\('\/dashboard\/mocks'\)\s*return \{ success: true \}\s*\}/;

const newCreate = `export async function createMockService(formData: FormData) {
...
if (code.match(regexCreate)) {
  code = code.replace(regexCreate, newCreate);
