...
export async function sendConfirmationEmail(record: any, serviceLabel: string) {
  ...
  const due = (record.registration_fee || 0) - (record.paid_amount || 0)

  const html = `
    <div style="font-family: Arial, sans-serif; padding: 20px; color: #333;">
      <h2 style="color: #2563eb;">YES Academy - Booking Confirmation</h2>
      <p>Dear <strong>${record.student_name}</strong>,</p>
      <p>Your <strong>${record.exam_type}</strong> has been successfully booked.</p>
      ...
      <div style="background-color: #f3f4f6; padding: 15px; border-radius: 8px; margin: 20px 0;">
        ...
        <p><strong>Total Fee:</strong> ৳${record.registration_fee || 0}</p>
