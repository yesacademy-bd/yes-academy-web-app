      ...
      {switchModal.isOpen && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xl max-w-md w-full p-6 border dark:border-slate-800">
            <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2"><Calendar className="w-5 h-5 text-amber-600" /> Switch Mock Date</h3>
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
              Student: <strong className="text-gray-900 dark:text-white">{switchModal.studentName}</strong><br/>
              Current Date: {new Date(switchModal.currentDate).toLocaleDateString()}
            </p>
            <div className="mb-6">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">New Exam Date</label>
              <input 
                type="date" 
                value={newDate} 
                onChange={e => setNewDate(e.target.value)} 
                className="w-full border-gray-300 dark:border-gray-600 dark:bg-slate-800 dark:text-white rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500" 
              />
            </div>
