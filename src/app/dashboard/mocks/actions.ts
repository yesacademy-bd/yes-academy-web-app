...
export async function updateMockService(id: string, formData: FormData) {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return { success: false, message: 'Unauthorized' }
  const { data: profile } = await supabase.from('profiles').select('role').eq('id', user.id).single()
  ...
  const amount = parseFloat(formData.get('amount') as string) || 0
  const paid_amount = parseFloat(formData.get('paid_amount') as string) || 0
  const payment_method = (formData.get('payment_method') as string) || (mock_status === 'Free' ? 'None' : 'Cash')
  const exam_date = formData.get('exam_date') as string
  const exam_time = formData.get('exam_time') as string
  const exam_venue = formData.get('exam_venue') as string
  const speaking_time = formData.get('speaking_time') as string || null
  ...
  if (student_type === 'Inhouse' && mock_status === 'Free') {
    const { count } = await adminClient.from('mock_services').select('*', { count: 'exact', head: true })
      .eq('phone', phone).eq('student_type', 'Inhouse').eq('mock_status', 'Free').neq('id', id)
    if (count !== null && count >= 2) return { success: false, message: 'Maximum limit passed.' }
  }

  const { count: sessionCount } = await adminClient.from('mock_services').select('*', { count: 'exact', head: true })
    .eq('service_type', mock_type).eq('exam_date', exam_date).neq('id', id)
  if (sessionCount !== null && sessionCount >= 10) return { success: false, message: 'This mock session is full.' }
