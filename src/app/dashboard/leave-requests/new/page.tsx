...
export default async function NewLeaveRequestPage({ searchParams }: { searchParams: { edit?: string } }) {
  ...
  if (searchParams.edit) {
    const { data } = await supabase.from('leave_requests').select('*').eq('id', searchParams.edit).single()
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{existingRequest ? 'Edit Leave Request' : 'New Leave Request'}</h1>
          <p className="text-gray-500 text-sm mt-1">{existingRequest ? 'Update your official leave request details.' : 'Submit an official leave request for approval.'}</p>
        </div>
      </div>
      ...
      <LeaveRequestClientForm profile={profile} initialData={existingRequest} />
    </div>
  )
}
